...
function chkFormulaire() {
 if(document.forms(0).data.value == "") {
  alert("<?php textdomain('plasmiddb'); print _("Search is empty!"); textdomain('phplabdb'); ?>");
  return false;
 };
};
    //]]>
...
<?php
  if (!(empty($data))) {
   ...
   if ($type=='name' || $type=='notes') {
    $string_search="SELECT name,length,circ,dna,plasmid FROM prototype WHERE $type" . sql_reg(addslashes($data)) . ' ORDER BY name;';
    $visual=true;    
   } elseif ($type=='seq' || $type=='alias') {
    $string_search="SELECT a.name,a.length,a.circ,a.dna,a.plasmid FROM prototype as a, $type as b WHERE (a.name=b.name AND b.$type" . sql_reg(addslashes($data)) . ') ORDER BY a.name;';
    $visual=true;
   } else {
    $string_search="SELECT name,released,box,rank,conditioning,preparation,barcode FROM plasmid WHERE $type" . (($type=='box')?('=' . addslashes($data)):(sql_reg(addslashes($data)))) . ' ORDER BY ' . (($type=='box')?('rank'):($type)) . ';';
   };
...
<?php 
      while($row=sql_fetch_row($result)) {
       print "            <tr class=\"" . (($i++%2 ==1) ? 'odd' : 'even' ) . "\">\n              <td class=\"column-1\">\n                &nbsp;<a href=\"" . $base_url . $plugin . 'details.php?plasmid='. rawurlencode($row[0]) . '" title="' . _("details") . '">' . $row[0] . "</a>&nbsp;\n              </td>\n              <td class=\"column-2\">\n                &nbsp;" . ((intval($row[1])>1)?$row[1]:'-') . ' ' . _("bp") . "&nbsp;\n              </td>\n              <td class=\"column-3\">\n                &nbsp;" .  (($row[2]=='t')? _("Circular"): _("Linear")) . "&nbsp;\n              </td>\n              <td class=\"column-3\">\n                &nbsp;" . (($row[3]=='d')? _("DNA"): _("RNA")) . "&nbsp;\n              </td>\n              <td class=\"column-3\">\n                <img src=\"images/" . (($row[4]=='t')?'yes':'no') . ".png\" alt=\"\" />\n              </td>\n            </tr>\n";
      };
?>
          </table>
<?php } else {?>
...
<?php 
      while($row=sql_fetch_row($result)) {
       print "            <tr class=\"" . (($i++%2 ==1) ? 'odd' : 'even' ) . "\">\n              <td class=\"column-1\">\n                &nbsp;<a href=\"" . $base_url . $plugin . 'details.php?barcode='. rawurlencode($row[6]) . '" title="' . _("details") . '">' . $row[0] . "</a>&nbsp;\n              </td>\n              <td class=\"column-2\">\n                &nbsp;$row[4]&nbsp;\n              </td>\n              <td class=\"column-2\">\n                &nbsp;$row[5]&nbsp;\n              </td>\n              <td class=\"column-3\">\n                &nbsp;" . date(_("m/d/Y"),strtotime($row[1])) . "&nbsp;\n              </td>\n              <td class=\"column-3\">\n                &nbsp;" . $row[2] . (($row[3]==0) ? '' : ('&nbsp;(' . $row[3] . ')')) . "&nbsp;\n              </td>\n            </tr>\n";
      };
?>
          </table>
<?php
