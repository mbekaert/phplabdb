...
<?php
  ...
  if (isset($_POST['barcode'])) {
   ...
   $result=sql_query("UPDATE config SET value=" . ((isset($_POST['primer']) && (intval($_POST['primer'])!=0))?("'".intval($_POST['primer'])."'"):'NULL') . " WHERE id='primer';",$dbconn);
  };
...
?>
          ...
          <form action="<?php print $base_url . $plugin; ?>admin/tips.php" method="post">
            <table id="list-database" width="100%" summary="">
              ...
              <tr class="odd">
                <td class="column-1">
                  &nbsp;2. <?php print _("Right primer minimum lenght?"); ?>&nbsp;
                </td>
                <td class="column-2">
                  <?php print '<input type="text" size="5" name="primer" value="' . ((isset($primer))?$primer:'') . '">&nbsp;'; ?> 
                </td>
                <td class="column-3">
                  <input type="submit" name="submit" value="<?php print _("Update"); ?>">
                </td>
