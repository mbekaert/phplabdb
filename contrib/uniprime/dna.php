...
if ($config['login'] && ((isset($_GET['sequence']) && preg_match('/^S(\d+)\.(\d+)/', $_GET['sequence'], $matches)) || (isset($_GET['mrna']) && preg_match('/^R(\d+)\.(\d+)/', $_GET['mrna'], $matches)) || (isset($_GET['primer']) && preg_match('/^X(\d+)\.(\d+)/', $_GET['primer'], $matches)))) {
  ...
  if (isset($_GET['primer'])) {
    ...
    $result = sql_query('SELECT a.penality, a.left_seq, a.left_data, a.left_name, a.right_seq, a.right_data, a.right_name, a.location, a.pcr, a.updated, a.comments, b.name, a.author, d.code, e.class, b.functions, c.sequences FROM uniprime_primer AS a, uniprime_locus AS b, uniprime_alignment AS c, users AS d, uniprime_class AS e WHERE a.prefix=' . octdec($matches[1]) . ' AND a.id=' . octdec($matches[2]) . ' AND b.prefix=a.locus_prefix AND b.id=a.locus_id  AND c.prefix=a.alignment_prefix AND c.id=a.alignment_id AND a.author=d.username AND b.class=e.id;', $sql);
    if ((!strlen($r = sql_last_error($sql))) && (sql_num_rows($result) == 1)) {
      ...
      print "\t\t<specificity>\n";
      $tree = array();
      foreach(explode(' ', $row[16]) as $seqs) {
        if (preg_match('/^(\d+)\.(\d+)/', $seqs, $seq)) {
          $result_seq = sql_query('SELECT a.accession, a.organism, a.sequence_type, a.geneid, b.taxonid, b.taxon FROM uniprime_sequence AS a, tree_taxonomy AS b WHERE a.prefix=' . octdec($seq[1]) . ' AND a.id=' . octdec($seq[2]) . ' AND a.organism=b.scientificname;', $sql);
          if ((!strlen($r = sql_last_error($sql))) && (sql_num_rows($result_seq) > 0)) {
            while ($row_seq = sql_fetch_row($result_seq)) {
              if (($row_seq[2]==1) || empty($targetid))  {
                $targetid=$row_seq[3];
                $taxonid=$row_seq[4];
                $sp=$row_seq[1];
              }
              $tree = deeptree($tree, explode(';', $row_seq[5] . ';' . $row_seq[4]));
            }
          }
        }
      }
      if (isset($tree) && (($species=trunctree($tree))!==false)) {
        print "\t\t\t<species>$species</species>\n";
      } else {
        print "\t\t\t<species" . (!empty($taxonid) ? (' id="' . $taxonid . '"') : '') . (!empty($sp) ? ('>' .$sp . '</species>') : ' />') . "\n";
      }
      print "\t\t\t<target" . (!empty($targetid) ? (' id="' .$targetid . '"') : '') . (!empty($row[11]) ? ('>' . $row[11] . '</target>') : ' />') . "\n";
      $data1 = explode('|', $row[2]);
      $data2 = explode('|', $row[5]);
    ...
    if (isset($_GET['sequence'])) {
      $result = sql_query('SELECT a.name, a.sequence, a.accession, a.location, a.organism, b.taxonid, a.features FROM uniprime_sequence AS a, tree_taxonomy AS b WHERE a.prefix=' . octdec($matches[1]) . ' AND a.id=' . octdec($matches[2]) . ' AND a.organism=b.scientificname;', $sql);
    }elseif (isset($_GET['mrna'])) {
      $result = sql_query('SELECT b.name, a.mrna, b.accession, a.location, b.organism, c.taxonid  FROM uniprime_mrna AS a, uniprime_sequence AS b, tree_taxonomy AS c WHERE a.prefix=' . octdec($matches[1]) . ' AND a.id=' . octdec($matches[2]) . ' AND b.prefix=a.sequence_prefix AND b.id=a.sequence_id AND b.organism=c.scientificname', $sql);
    }
    if (strlen($r = sql_last_error($sql)) || (sql_num_rows($result) < 1)) {
