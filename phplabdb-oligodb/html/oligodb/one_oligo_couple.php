<?php
 ...
 function printer($liste_oligo) {
  $i=0;
	if(isset($liste_oligo[7])) {
   foreach($liste_oligo[7] as $newcouple) {
 ...
 function affiche_recherche($data,$type,$j,$lister) {
  ...
  if (!(strlen($r=sql_last_error($dbconn)))) {
   ...
   if ($nombre_membre!=0) {
    ...
    print '<tr><td colspan="3"><em>' . _("No result") . '</em></td></tr>';   
   };
  };
 };
...
<?php  textdomain('oligodb'); ?>
          ...
          <form method="post" action="<?php print $base_url . $plugin; ?>one_oligo_couple.php">
...
<?php
  if ((isset($_POST['oligo'])) && (!(isset($_POST['type'])))) {
   ...
   if(isset($liste_oligo[7])){
    ...
    foreach($liste_oligo[7] as $newcouple) {
     if(!(isset($oligos[$i++]))) {
      unset($liste_oligo[7][array_search(rawurldecode($newcouple),$liste_oligo[7])]);
     };
    };
   };
   if(isset($_POST['couple'])){
    foreach($_POST['couple'] as $newcouple) {
     $liste_oligo[7][]=rawurldecode($newcouple);
    };
   };
   $liste_oligo[7]=array_unique($liste_oligo[7]);
   $_SESSION['oligo_liste']=$liste_oligo;
   $j=printer($liste_oligo);
   if (!(empty($_POST['data']))){
    affiche_recherche($_POST['data'],$_POST['type'],$j,$liste_oligo[7]);
   };
  };
?>
            </table>
