...
<?php
  if (!(empty($data))) {
   $result=sql_query("SELECT name, oligo, synthesis, box, rank, freezer, barcode, notes FROM oligo WHERE $type" . (($type=='box')?("='". addslashes(htmlentities($data)) ."'"):(sql_reg(addslashes(htmlentities($data))))) . " ORDER BY " . (($type=='box')?('rank'):($type)) . ';',$dbconn);
...
<?php
     while($row=sql_fetch_row($result)) {
       ...
       if(!(strlen($r=sql_last_error($dbconn)))) {
        ...
        if ($nombre_membre2!=0) {
         ...
         while($row2=sql_fetch_row($result2)){
          print '<a href="' . $base_url . $plugin . 'search.php?type=name&amp;data=' . rawurlencode(addslashes((($row2[0]==$row[0])?$row2[1]:$row2[0]))) . '">' . (($row2[0]==$row[0])?$row2[1]:$row2[0]) . '</a> ';
         };
         print "&nbsp;\n              </td>\n            </tr>\n";
        };
       };
      ...
      if(isset($note)) {
       if ((strlen($row[7])!=0) && (trim($row[7])!='NULL')){
        print "            <tr class=\"" . (($i%2==1) ? 'odd' : 'even' ) . "\">\n              <td class=\"column-1\">\n              </td>\n              <td colspan=\"3\">\n                &nbsp;" . ((strlen($row[7])>60)?(substr($row[7],0,60).'...'):($row[7])) .  "&nbsp;\n              </td>\n            </tr>\n";
       };
      };
      $i++;
     };
