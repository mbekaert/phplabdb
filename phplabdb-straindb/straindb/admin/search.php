...
.strain { background: url('../images/strain.png') no-repeat right top; }
 ...
 function confirmation(){
  if (confirm('<?php textdomain('straindb'); print _("Remove?"); textdomain('phplabdb'); ?>')){
   return true;
  } else {
   return false;
  };
...
<?php
  if (!(empty($data))) {
   ...
   $result=sql_query("SELECT name, barcode, released, box, rank, ploidy, species FROM strain WHERE $type" . (($type=='box')?("='". addslashes($data) ."'"):(sql_reg(addslashes($data)))) . " ORDER BY " . (($type=='box')?('rank'):($type)) . ';',$dbconn);
   if(!(strlen($r=sql_last_error($dbconn)))) {
...
<?php
     while($row=sql_fetch_row($result)) {
      print "            <tr class=\"" . (($i++%2 ==1) ? 'odd' : 'even' ) . "\">\n              <td class=\"column-1\">\n                &nbsp;$row[0]&nbsp;\n              </td>\n              <td class=\"column-2\">\n                &nbsp;<em>" . $species[$row[6]] . "</em>&nbsp;\n              </td>\n              <td class=\"column-2\">\n                &nbsp;<small>$row[5]</small>&nbsp;\n              </td>\n              <td class=\"column-3\">\n                &nbsp;" . $row[3] . (($row[4]==0) ? '' : ('&nbsp;(' . $row[4] . ')')) . "&nbsp;\n              </td>\n              <td class=\"column-6\">\n                &nbsp;<a href=\"modif.php?barcode=" . rawurlencode($row[1]) . '">' . _("Modify") . '</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="modif.php?barcode=' . rawurlencode($row[1]) . '&amp;remove=1" onclick="return confirmation();">' . _("Remove") . "</a>&nbsp;\n              </td>\n            </tr>\n";
     };
     print "          </table>\n";
    } else {
     print "          <p>\n            <em>" . _("No result") . "</em>\n          </p>\n";
